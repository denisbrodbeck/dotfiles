...
say_info 'Configuring Ubuntu 20.04 for DEV environment'
...
sudo apt -q -y install fish curl nano gnome-tweaks open-vm-tools-desktop ripgrep \
                        ansible shellcheck \
                        git subversion \
                        gcc clang make cmake \
                        dotnet-sdk-3.1 \
                        python3 \
                        php-cli php-curl php-gd php-imagick php-pdo php-pgsql php-intl php-json php-xml php-mbstring php-bcmath php-opcache
DEBIAN_FRONTEND=noninteractive sudo apt -q -y install wireshark

mkdir -p ~/.local/bin

# download nice fonts
say 'install fonts'
mkdir -p -m 0775 ~/.local/share/fonts
...
wget -q https://update.code.visualstudio.com/latest/linux-deb-x64/stable -O /tmp/setup_code.deb
sudo apt -q -y install /tmp/setup_code.deb
